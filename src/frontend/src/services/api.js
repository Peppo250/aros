  ...
  getProjects: async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        const dbRuns = response.data.map(proj => ({
          run_id: proj.id,
          project_id: proj.id,
          topic: proj.topic || proj.title || 'Untitled Research',
          status: 'completed',
          created_at: proj.created_at || new Date().toISOString()
        }));
        
        // Merge: keep localRuns (which have active status/progress) and append missing dbRuns
        const merged = [...localRuns];
        dbRuns.forEach(dRun => {
          if (!merged.some(m => m.project_id === dRun.project_id || m.run_id === dRun.run_id)) {
            merged.push(dRun);
          }
        });
        return merged;
