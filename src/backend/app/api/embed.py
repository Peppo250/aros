...
def embed_project(
    ...
    chunks = (
        ...
        .filter(
            ...
            or_(
                DocumentChunk.paper_id.in_(
                    db.query(Paper.id).filter(Paper.project_id == project_id)
                ),
                DocumentChunk.patent_id.in_(
                    db.query(Patent.id).filter(Patent.project_id == project_id)
                )
            )
        )
        .all()
