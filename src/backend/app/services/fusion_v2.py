...
def extract_json(text: str) -> dict:
    ...
    return extract_json_from_text(text, text_fields=fields, confidence_field="confidence")
...
def generate_fusion_report_v2(
    ...
    patents_total = db.query(Patent).filter(Patent.project_id == project_id, Patent.is_verified == True).count()
